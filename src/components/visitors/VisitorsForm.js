...
class VisitorForm extends Component {
  ...
  async componentDidMount() {
    await this.props.getStaff(1, 500);
  ...
  renderStaffSelect() {
    ...
    return (
      <div className="form-group">
        <label htmlFor="satff"> Hostj</label>
        <input
          type="search"
          name="query"
          value={query}
  ...
  handleBackClick() {
    this.props.setCurrent(this.state.step - 1);
  }
  // render company selection for workspace use case
  renderCompanySelect() {
      ...
      return (
        <form onSubmit={this.handleSubmit} autoComplete="false">
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <Button color="danger">{isLoading ? this.renderSpinner() : 'Next'}</Button>
          </div>
        </form>
      );
    return null;
